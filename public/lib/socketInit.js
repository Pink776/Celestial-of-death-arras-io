...
var serverStart = 0,
    gui = {
        getStatNames: data => [
            data?.body_damage ?? 'Body',
            data?.max_health ?? 'Max',
            data?.bullet_speed ?? 'Bullet',
            data?.bullet_health ?? 'Bullet Heal',
            data?.bullet_pen ?? 'Bullet Pen',
            data?.bullet_damage ?? 'Bullet Dmg',
            data?.reload ?? 'Reload',
            data?.move_speed ?? 'Movement',
            data?.shield_regen ?? 'Shield',
            data?.shield_cap ?? 'Shield Cap',
        ],
        skills: [
