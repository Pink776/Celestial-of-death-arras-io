...
Class.auraDroneGen = addAura(0.65);
Class.auraDrone = {
  PARENT: ["drone"],
  TURRETS: [
    {
...
//corps
let corpss = new LayeredBoss(null, "Corpss", "CorpsEternal", 30, "red", "baseTrapTurret", 4, 10.5);
corpss.addLayer({turret: {
  POSITION: [4, 8, 0, null, 160, 0],
  TYPE: "desmosTurret",
}}, true, 10.5);
corpss.addLayer({turret: {
  POSITION: [4, 8, 0, null, 160, 0],
...
Class.foregiLauncher = {
  PARENT: ["launcher"],
   CONTROLLERS: [ "canRepel", "onlyAcceptInArc", "mapAltToFire", "nearestDifferentMaster" ],
};

let foreginn = new LayeredBoss(null, "Foreginn", "eternal", 13, "black", "baseTrapTurret", 4, 5.5);
foreginn.addLayer({turret: {
  POSITION: [6,8,0,null,160,0],
  TYPE: "foregiGunner",
}}, true, 5.5);
foreginn.addLayer({turret: {
  POSITION: [6,8,0,null,160,0],
...
let criston = new LayeredBoss(null, "Criston", "celestial", 9, "red", "baseTrapTurret", 6.5, 5.5);
criston.addLayer({gun: {
    POSITION: [3.8, 6, 1.4, 8, 0, null, 1],
    PROPERTIES: {
        SHOOT_SETTINGS: combineStats([g.drone, g.sunchip, { size: 0.5 }]),
        TYPE: ["summonerDrone", {INDEPENDENT: true}],
...
criston.addLayer({turret: {
    POSITION: [10, 7.5, 0, null, 160, 0],
    TYPE: ["artilleryTurret", {COLOR: "grey"}],
}}, true, 6);
let gottae = new LayeredBoss(null, "Gottae", "celestial", 9, "green", "baseTrapTurret", 6.5, 5.5);
gottae.addLayer({gun: {
    POSITION: [3.8, 6, -1.4, 8, 0, null, 1],
    PROPERTIES: {
        SHOOT_SETTINGS: combineStats([g.drone, g.sunchip, { size: 0.5 }]),
...
gottae.addLayer({turret: {
    POSITION: [10, 7.5, 0, null, 160, 0],
    TYPE: ["desmosTurret", {COLOR: "grey"}],
}}, true, 6);
let jurito = new LayeredBoss(null, "Jurito", "celestial", 9, "brown", "baseTrapTurret", 6.5, 5.5);
jurito.addLayer({gun: {
    POSITION: [3.8, 6, -1.4, 8, 0, null, 1],
    PROPERTIES: {
        SHOOT_SETTINGS: combineStats([g.drone, { size: 0.5 }]),
        TYPE: ["auraDrone", {INDEPENDENT: true}],
        AUTOFIRE: true,
        WAIT_TO_CYCLE: true,
        SYNCS_SKILLS: true,
    },
}}, true, null, 35);
jurito.addLayer({turret: {
    POSITION: [10, 7.5, 0, null, 160, 0],
    TYPE: ["poundTurret", {COLOR: "grey"}],
}}, true, 6);
let incast = new LayeredBoss(null, "Incast", "celestial", 9, "blue", "baseTrapTurret", 6.5, 5.5);
incast.addLayer({gun: {
    POSITION: [3.8, 6, -1.4, 8, 0, null, 1],
    PROPERTIES: {
        SHOOT_SETTINGS: combineStats([g.drone, { size: 0.5 }]),
...
incast.addLayer({turret: {
    POSITION: [10, 7.5, 0, null, 160, 0],
    TYPE: ["auto4", {COLOR: "grey"}],
}}, true, 6);
let kopop = new LayeredBoss(null, "Kopop", "eternal", 11, "pink", "baseTrapTurret", 4.5, 3.5);
kopop.addLayer({gun: {
...
let cripto = new LayeredBoss(null, "Cripto", "eternal", 19, "lavender", "baseTrapTurret", 3.5, 19.5);
cripto.addLayer({gun: {
    POSITION: [2.25, 3.25, 1.6, 9, 0, null, 0],
    PROPERTIES: {
        SHOOT_SETTINGS: combineStats([g.factory, { size: 0.5 }, g.pounder, {size: 1.7}]),
...
//My Bosses versus celestials



Class.gurritAura = addAura(6, 1.5, 1.3);
